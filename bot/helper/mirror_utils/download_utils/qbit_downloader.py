    ...
    def add_torrent(self, link, dire, listener, qbitsel):
        ...
        try:
            ...
            if BASE_URL is not None and qbitsel:
                ...
                if WEB_PINCODE:
                    buttons.buildbutton("Select Files", f"{BASE_URL}/snake/files/{self.ext_hash}")
                    buttons.sbutton("Pincode", f"pin {self.gid} {self.pincode}")
                else:
                    buttons.buildbutton("Select Files", f"{BASE_URL}/snake/files/{self.ext_hash}?pin_code={self.pincode}")
